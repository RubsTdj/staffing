...
import { StaffingStatus, StaffingDot } from "@/components/ui/staffing-status";
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-start justify-between gap-3 border-b border-[var(--color-line)] px-6 pt-5 pb-4">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            ...
            />
            <ValidationStatus kind={activity.validation} />
            <StaffingStatus status={activity.status} />
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
        ...
        <section className="grid grid-cols-2 gap-3">
          <Meta icon={<Calendar size={13} strokeWidth={1.6} />} label="Date">
          ...
          <Meta icon={<MapPin size={13} strokeWidth={1.6} />} label="Centre">
            {centre?.address}
          </Meta>
          <Meta
        ...
        <section className="rounded-lg border border-[var(--color-line)] bg-white p-3.5">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-[15px] tracking-tight text-[var(--color-ink)]">
              {required > assigned.length ? (
                <>
                  <span className="font-medium">{required - assigned.length}</span>{" "}
                  accompagnant{required - assigned.length > 1 ? "s" : ""} requis
                </>
              ) : (
          ...
          <div className="mt-1 text-[11.5px] text-[var(--color-ink-3)]">
            Règle : 1 accompagnant pour 9 salariés ({centre?.nbSalaries ?? "?"}{" "}
            salariés sur ce centre).
          </div>
        ...
        <section>
          <div className="mb-2 flex items-center justify-between">
            <h3 className="text-[12px] font-medium uppercase tracking-[0.14em] text-[var(--color-ink-3)]">
              Équipe assignée
            </h3>
          ...
          <div className="mt-3 rounded-lg border border-[var(--color-line)] bg-white">
            <div className="flex items-center gap-2 border-b border-[var(--color-line)] px-2.5 py-1.5">
              <Plus size={13} strokeWidth={1.8} className="text-[var(--color-ink-3)]" />
              <input
                value={query}
            ...
            <div className="mt-3 flex items-start gap-2 rounded-lg border border-[var(--color-status-alert)]/30 bg-[var(--color-tint-pink)] px-3 py-2.5 text-[12px] text-[var(--color-tint-pink-ink)]">
              ...
              <div>
                ...
                <div className="opacity-90">
                  Impossible d'ajouter {obsAlert} : un seul observateur est autorisé
                  par déplacement.
                </div>
                <button
                  type="button"
        ...
        {/* Workflow */}
        <section>
          <h3 className="mb-2 text-[12px] font-medium uppercase tracking-[0.14em] text-[var(--color-ink-3)]">
            Workflow
          </h3>
